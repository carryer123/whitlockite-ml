...
# ---- Full-fit: feature importance + ranking ----
...
print(f"Predicted hardness ranking (XGBoost): {' > '.join(rank)}")
print(f"Experimental ranking:                 Ni > Co > Mg > Cu")
print(f"\nNOTE: with n=4 the LOO point predictions are statistically limited; the robust"
      f"\nconclusions are (i) CFSE dominates feature importance and (ii) the rank order.")
print(f"Saved: {os.path.join(OUT, 'loocv_hardness_xgboost.csv')}")
